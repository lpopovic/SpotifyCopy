...
struct PlayerControlsViewViewModal {
    let title: String?
...
final class PlayerControlsView: UIView {
    ...
    @objc private func didTapPlayPause() {
        ...
        // Update icon
        let image = UIImage(systemName: self.isPlaying ?  "pause" : "play.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: 34, weight: .regular))
        playPauseButton.setImage(image, for: .normal)
    }
    
    func configure(with viewModal: PlayerControlsViewViewModal) {
        nameLabel.text = viewModal.title
        subtitleLabel.text = viewModal.subtitile
    }
}
